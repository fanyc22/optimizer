...
app = typer.Typer(help="Hardware/software topology search optimizer CLI.", no_args_is_help=True)
console = Console()
...
def tgrl_optimizer(
    ...
    seed_archive_size: int = typer.Option(
        16,
        min=1,
    ...
    seed_diversity_attempts: int = typer.Option(
        12,
        min=1,
        help="Attempts to find an unseen diversified TG-RL v2 env seed.",
    ),
    mask_seen_actions: bool = typer.Option(
        True,
        help="Mask TG-RL v2 actions whose resulting candidate was already seen or cached.",
    ),
    restart_on_stall: bool = typer.Option(
        True,
        help="Restart a TG-RL v2 env from a diversified seed when all actions are already seen.",
    ),
    skip_cache_hit_transitions: bool = typer.Option(
        True,
        help="Do not train PPO on TG-RL v2 transitions that were cache hits.",
    ),
    ...
    if mode == "v2":
        ...
        runner_v2 = TGRLPPOTrainer(
            ...
            config=TGRLPPOConfig(
                ...
                seed_archive_size=seed_archive_size,
                seed_diversity_steps=seed_diversity_steps,
                seed_diversity_attempts=seed_diversity_attempts,
                mask_seen_actions=mask_seen_actions,
                restart_on_stall=restart_on_stall,
                skip_cache_hit_transitions=skip_cache_hit_transitions,
                kl_weight=kl_weight,
